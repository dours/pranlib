...
(* creates array of parents in tree (using post number!) *)
  ...
  let frw_compute_reverse_tree dfs g =
    ...
    List.iter (fun el -> if dfs.DFST.sort el == DFST.Tree then (
                           arr.((dfs.DFST.pre (G.dst el)) + 1)<-
                               ((dfs.DFST.pre (G.src el)) + 1))) (G.edges g);
    arr

  let apply_int from tooo f = 
    let rec apply_n n =
      if (n<=tooo) then apply_n (n+1);f n
    in apply_n from
...
(*sophisticated implementation of fast algorithm*)
  ...
  let create g dfs = 
    ...
    let link v w =                 (* link implementaion *) 
      ...
      let s = if size.(v) < 2 * size.(w) then ( 
                let tmp = child.(v) in
                child.(v) <- s;
                tmp
              ) else s in 
      let rec iter1 s =
        if s <> 0 then (ancestor.(s) <- v; iter1 child.(s) ) in
      iter1 s in
    let eval v =                   (* eval implementaion *)
      ...
      if anc_v_n == 0  then 
        ...
        if semi.(l_anc_v) >= semi.(label_v) then 
          label_v
        else 
          l_anc_v in
    for i = g_size downto 2 do (*implicity calculation*)
      printf "implicit %i\n" i;
      let w = dfs.DFST.pre'1 (i-1) in
      let parent_i = parent.(i) in
      printf "    parent %i\n" parent_i;
      List.iter (fun v_eg ->let u = eval (1 + dfs.DFST.pre (G.src v_eg)) in
                            let semi_u = semi.(u) in 
                            if semi_u < semi.(i) then
                               semi.(i) <- semi_u) (G.ins w); 
      ...
      List.iter (fun v -> 
                  let u = eval v in
                  dm.(v) <- if semi.(u) < semi.(v) then (printf "  dm %i <- %i\n" v u; u) else (printf "  dm %i <- %i\n" v parent_i; parent_i))
                bucket.(parent_i);
      bucket.(parent_i) <- [] 
    done;
    for i = 2 to g_size do (*explicit calculations*)
      let w = dfs.DFST.pre'1 (i-1) in
      let dom_i = dm.(i) in
      if  dom_i <> semi.(i) then (     
        dm.(i) <- dm.(dom_i)
      )
    done;
