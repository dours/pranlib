...
module LOOP (G: Digraph.Sig) (UFS : Unionfind.S with type elt=G.Node.t) (UFSI : Unionfind.S with type elt=int) = 
struct
  ...
  let collapse uf li body header =
    ...
    HSHND.iter (fun x y -> LOG (printf "    with: %s\n" (G.Node.toString x)); UFS.union header x uf; HSHND.add li x header)
       body
...
(* Simple Havlak implementation *)
  let loops_havlak g df =
  ...
  (* Improved Havlak implementation *)
  let loops_havlak_i g df = 
     ...
     let add_to_mult_hash hsh key el =  
        ...
        else HSHND.add hsh key [el] 
     in

     (* creates array of parents in tree (using post number!) *)
     let compute_reverse_tree dfs g = 
         let arr = HSHND.create (G.nnodes g) in
         HSHND.add arr dfs.DFST.start dfs.DFST.start;   
         List.iter (fun el -> if dfs.DFST.sort el == DFST.Tree then HSHND.add arr (G.dst el) (G.src el)) (G.edges g);
         arr 
     in
...
let loops_gao_lee g dfs = 
    ...
    let create_node nd = 
        ...
        loops.(!cur) <- List.map (fun x -> 
           let x = UFSI.find (dfs.DFST.pre (G.src x)) uf in
           LOG (printf "      incoming from %i \n" x);
           x
        ) (G.ins (dfs.DFST.pre'1 nd));
        incr (cur);
        !cur - 1 
    in
        ...
        if work_list <> []  then (
           ...
           | hd :: tl_wl -> 
               ...
               let precedors =
                  ...
                  else List.map (fun x -> UFSI.find (dfs.DFST.pre (G.src x)) uf) (G.ins (dfs.DFST.pre'1 hd)) 
               in
    ...
    in
    let f1 f_not nd = 
        if f_not true then LOG (printf "red   find for node %i\n" nd) 
        else LOG (printf "irred find for node %i\n" nd);

        let w = List.fold_left (fun lst el_eg ->
                   let el_nd = G.src el_eg in
                   let el_nd_n = dfs.DFST.pre el_nd in
                   LOG (printf "     incoming from %i\n" el_nd_n);
                   if dfs.DFST.sort el_eg == DFST.Back && f_not (dm.DOMM.INT_TREE_BUILDER.is_child el_nd_n nd)  
