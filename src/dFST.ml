...
module Make (G : Digraph.Sig) (X : sig val graph : G.t val start : G.Node.t end) =
    ...
    let build graph =
      ...
      let pre   = Array.make (G.lastNode graph) (-1) in
      let post  = Array.make (G.lastNode graph) (-1) in
      let sort  = Array.make (G.lastEdge graph) Tree in
      let state = Array.make (G.lastNode graph) `Init in
      
      let set p p' node num =
    p.(G.Node.index node) <- num;
    p'.(num) <- node
      in
      
      let setPost node num = set post post' node num in
      let setPre  node num = set pre  pre'  node num in
      let setSort edge s   = sort.(G.Edge.index edge) <- s in
      
      let reachedNode node = pre.(G.Node.index node) <> -1 in
      let reachedEdge edge = reachedNode (G.src edge) in

      let isValid i =   
    (i >= 0) && (i < n) && (i = 0 || not (G.Node.equal pre'.(i) start))
      in

      let getPre node = 
    let m = pre .(G.Node.index node) in
    if m = -1 then raise (Unreachable (`Node node)) else m
      in

      let getPost node = 
    let n = post.(G.Node.index node) in
    if n = -1 then raise (Unreachable (`Node node)) else n
      in

      let getSort edge = 
    if reachedEdge edge then sort.(G.Edge.index edge) else raise (Unreachable (`Edge edge))
      in

      let getPre'  num = if isValid num then pre' .(num) else raise (RangeError num) in
      let getPost' num = if isValid num then post'.(num) else raise (RangeError num) in
      
      let rec visit (currM, currN) = function
    | [] -> ()
    | (node, []) :: tl ->
        setPost node currN;
        state.(G.Node.index node) <- `Done;
        visit (currM, currN-1) tl
          
    | (node, nextEdge :: rest) :: tl ->
        let nextNode = G.dst nextEdge in
        begin match state.(G.Node.index nextNode) with
        | `Init ->
        setPre nextNode currM;
        state.(G.Node.index nextNode) <- `InProcess;
        visit 
          (currM+1, currN) 
      ...
      in
      state.(G.Node.index start) <- `InProcess;
      setPre start 0;
    ...
    module Edge =
      struct
        ...
        let attrs edge = match sort edge with
        | Tree    -> ["color", "black"; "style", "bold"]
        | Cross   -> ["color", "blue"]
        | Forward -> ["color", "green"]
        | Back    -> ["color", "red"]
          
        let label edge = sprintf "%s\n%s" 
