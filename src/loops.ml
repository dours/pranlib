	  ...
	  | hd :: tl ->
		...
		fold_left 
		  (fun (scc, visited, frontier) edge ->
		    ...
		    LOG (Printf.fprintf stderr "    Considering candidate %d..." (T.post candidate));
		    if not (NodeSet.mem candidate visited) && (T.post candidate > binumber)
		    then begin
	...
	module SCS =
	  struct
	    ...
	    let all = 
		  ...
		  (fun i -> 
		    if T.isValid i 
		    then lazy (snd (region NodeSet.empty i [] [T.post'1 i]))
		    else lazy []
		  )
	      )

	    let get node = Lazy.force (Lazy.force all).(T.post node)
...
(*
	...
	let build () = 
	  ...
	  let is_in_subtree hd nd =
            ...
            let hd1 = T.post hd_node in 
            let nd1 = T.post nd_node in
            hd <= nd && hd1 <= nd1 
	  in  
	  let create_node nd = 
            LOG (
