  ...
  let create info =
     ...
     let itr el el_par =
         ...
         then
             ...
             HSHT.add tree_inverse el el_par
         ) 
     in
     info.iter itr;
     let rec build_index node pre post =
         if HSHT.mem tree_table node 
         then (
            let cur_el = match HSHT.find tree_table node with (c_el, _, _) -> c_el in
            let ch_n = List.fold_left (fun cur_ch_n son -> 
                                          cur_ch_n + build_index son (pre+1+cur_ch_n) (post - cur_ch_n)
                                     ) 0 cur_el 
            in
            HSHT.replace tree_table node (cur_el, pre, post - ch_n);
            ch_n+1
         ) 
         else (
            HSHT.add tree_table node ([],pre,post);
            1
         )
     in 
     ...
     {
      get_childs = (fun el -> 
                       if HSHT.mem tree_table el 
                       then match HSHT.find tree_table el with (el_childs, _, _) -> el_childs
                       else [] 
                   );

      get_parent = (fun el -> 
                        if HSHT.mem tree_inverse el then Some (HSHT.find tree_inverse el)
                        else None 
                   );
