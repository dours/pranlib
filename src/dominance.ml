  ...
  struct
	...
	let build () = 
	  ...
	  let parent     = 
		...
		then arr.((T.pre (G.dst el)) + 1) <- ((T.pre (G.src el)) + 1)
	      )
              (D.G.edges g);
	    arr
	  in
	  ...
	  for i = g_size downto 2 do 
            ...
            let w = T.pre'1 (i-1) in
            let parent_i = parent.(i) in
            LOG (printf "    parent %i\n" parent_i);
            List.iter 
	      (fun v_eg ->
		let u = eval (1 + T.pre (G.src v_eg)) in
	...
	let parent node = 
	  ...
	  then None
	    ...
	    Some (T.pre'1 (dm.((T.pre node) - 1) + 1))
	      
	let children node =
	  let index = snd (Lazy.force data) in
	  let _, _, list = index.(T.pre node) in
	  List.map D.pre'1 list
	    
	let dominates x y =
	  let index = snd (Lazy.force data) in
	  let nx, mx, _ = index.(T.pre x) in
	  let ny, my, _ = index.(T.pre y) in
	  ny >= nx && ny <= nx + mx 
